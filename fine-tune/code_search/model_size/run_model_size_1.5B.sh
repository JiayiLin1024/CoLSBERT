...
for lang in ${languages[@]}
    do
        echo $lang
        echo "*** start fine-tune on 1.5B model ***"

        output_dir=fine-tune/code_search/saved_models/model_size/1.5B/$lang
        mkdir -p $output_dir
        saved_pretrained=pre-train/saved_models/model_size/1.5B

        torchrun --rdzv_backend=c10d --rdzv_endpoint=localhost:22302 --nproc_per_node=8 --nnodes=1 run_fsdp_PreLN.py \
